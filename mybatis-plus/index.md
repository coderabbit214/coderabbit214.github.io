...
public class CodeGenerator {
    ...
    public void run() {
        ...
        gc.setOutputDir("E:\\JAVA存储\\项目\\前后端总结\\template" + "/src/main/java");
        ...
        dsc.setUrl("jdbc:mysql://localhost:3306/template?serverTimezone=GMT%2B8");
        ...
        pc.setParent("com.jsh");
        pc.setModuleName("template"); //模块名
        ...
        strategy.setInclude("role");//表名，可以加入多张表 用","隔开
...
```java
QueryWrapper<StrategyCatalog> strategyCatalogQueryWrapper = new QueryWrapper<>();
strategyCatalogQueryWrapper.groupBy("dest_name").select("dest_name","group_concat(name) as names","group_concat(id) as ids");
List<Map<String, Object>> maps = baseMapper.selectMaps(strategyCatalogQueryWrapper);
//通过map的key去拿值
